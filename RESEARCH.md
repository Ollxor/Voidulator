...
| ["Creation" — Silexars (Danilo Guanabara)](https://www.shadertoy.com/view/XsXXDn) | **CC BY-NC-SA 3.0** (Shadertoy default) ⚠️ | One of the most famous tiny shaders ever. Its signature look — soft chromatic gradients — comes from computing **each RGB channel at a slightly offset phase**, a generic technique (free math). Voidulator's wave-field **Prism** colour mode (v1.20.0) applies that per-channel phase-offset idea to our own FDTD displacement — independently implemented, no code reuse. |
