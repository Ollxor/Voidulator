...
- **Contain in room (`S.trails.containWalls`, v1.32).** The Feedback transform above is a pure UV scale/rotate/shift with no concept of the room shape, so a sustained zoom pulls trail content out to ever-larger radii — and since `kfold()` (Kaleidoscope fold, above) preserves radius-from-centre and only folds angle, escaped content folds straight back into the visible pattern instead of vanishing off-frame. Fix is a hard clip, not a reflection: `buildTrailMask()` rasterizes the current room shape (circle or polygon, via `S.isCircle`/`S.circle`/`S.vertices` — same technique as `buildFieldMask()`, but framed directly in the trail buffer's own square-canvas UV space, since `buildShape()` always centres the room at `(W/2,H/2)` with a fixed margin, so no padding/centroid remap is needed) into `trailMaskTex`, cached on a geometry key exactly like the field mask. `trailFs` samples it and forces **output** pixels outside the mask to black unconditionally, every frame, gated behind `u_containWalls` — this must run before the feedback-transform sampling, not on the transformed `uv`, so it also scrubs anything already-escaped in the history buffer rather than just blocking new leakage. True per-pixel wall-bounce reflection was considered and rejected: it'd need a new wall-normal field (nothing like it exists), can't recover a true bounce direction from a pixel's position alone, and would only ever be a corner-case-heavy approximation — clipping is exact and cheap for what was actually reported (trails escaping the room), so that's what shipped. Off by default; existing scenes render unchanged.
