...
- Wall bending: `buildShape()` caches unbent corners in `S.baseVertices`; `bendWalls()` re-derives `S.vertices` via `bendVerts()` (24 subdivisions/wall, sine bulge along the outward normal). Always change bend through `bendWalls()`, never by calling `buildShape()` — the latter re-randomizes Randomgon. Circle, blob, and parabolic are exempt (`baseVertices` null).
