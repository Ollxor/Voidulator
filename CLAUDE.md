...
- **i18n gotcha (bit us in v1.24):** `hintTrails` (and any other multi-line `.hint` text) is duplicated across the HTML default *and* all 4 `TRANSLATIONS[lang]` tables. Editing only the HTML default silently regresses the moment a user selects any language (including re-selecting English) — `applyTranslations()` overwrites `textContent` from the table, stale or not. When changing a hint/tooltip string, grep for its i18n key across all languages and update every occurrence, not just the HTML.
